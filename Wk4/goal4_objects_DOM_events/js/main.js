...
//{KEY : Value} pairings,
	...
	//object person - defining name(storing bond), age (storing 35), secrentAgent(storing true)

	person = {name:'bond', age:35, secretAgent:true};
	//object person, keys are not in string name: 'bond' value is in a string.
 ...
 */
	var personAge = 'age';
							//index notation using var
	console.log(person.name, person[personAge], person['secretAgent']);
			//dot notation    					 // index notation using string

// setter
	person['age'] = '40';
	//if age wasn't present, it would then create this and set it to 40
	person.name = "JamesBond";
	console.log(person);
	//name changed to JamesBond and age changed to 40


//nested objects
	person = {birthday:{month:02, day:12}, name:'bond'};
	console.log(person); //outputting birthday - nested
	console.log(person['birthday']['month']); //outputting the birthday - not nested //using strings
...
console.log('------Object within an object, Arrays, Function ----------');
	...
	console.log(thatGuy.showMyAddress()); //target thatGuy object, and run the showMyAddress function

	console.log(thatGuy.address.street + ',' + thatGuy.address.city); // without using showMyAddress function
...
//Method 1 "Basic"
	var fsStudent = {};
	fsStudent.age = 22; // setter ; setting age, and setting it to 22
	fsStudent.career = 'Web Dev'; // setter ; setting career and setting it to 'web dev'
...
// 	by using their name as a string - all of the below are valid:
	...
	fsStudent.sayHello(); //run the method within fsStudent, sayhello //using dot notation
	fsStudent['sayHello'](); // using string ^^

	console.log(fsStudent.age); //output age in dot notation
	console.log(fsStudent['age']); //output age using string
