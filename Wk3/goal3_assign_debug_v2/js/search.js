...
(function(){
	...
	var validate = function(query){  //corrected spelling and deleted =
		...
		if(query.length < 3){
			alert("Your search query is too small, try again."); // added " to end
	...
	var search = function(query){
		...
		var queryArray = query.split(''); //changed to split
		...
		if(results.length === 0){ //to few = signs
			noMatch();
		}else{
			showMatches(results);
		}
	...
	document.forms[0].onsubmit = function() {
		...
		validate(query); // validate was mis-spelled
